...
def main():
	...
	i = args.ssim_num
	train(i, original_imgs_path, args)


def train(i, original_imgs_path, args):
	...
	if args.resume is not None:
		...
		ResCCNet_atten_model.load_state_dict(torch.load(args.resume))
	ssim_loss = pytorch_msssim.msssim
	mse_loss = torch.nn.MSELoss()


	if args.cuda:
		ResCCNet_atten_model.cuda()
	...
	for e in tbar:
		...
		print( "\nlearning_rate:",lr)
		print()
		optimizer = Adam(ResCCNet_atten_model.parameters(), lr)
		image_set, batches = utils.load_dataset(original_imgs_path, batch_size)
		ResCCNet_atten_model.train()
		count = 0
